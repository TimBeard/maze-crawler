...
export class DungeonGenerator {
    ...
    public generate(seed?: number, forcedSpawn?: SpawnPoint): number[][] {
        ...
        if (forcedSpawn && this.isValid(forcedSpawn.x, forcedSpawn.y)) {
            ...
            if (this.isValid(nextX, nextY)) {
                ...
                if (possibleDirections.length > 0) {
                    initialDirection = possibleDirections[Math.floor(this.random() * possibleDirections.length)]
                } else {
        ...
        // Vérifie que le spawn est bien dans un cul-de-sac, sinon en trouve un nouveau
        this.ensureSpawnIsDeadEnd()
    ...
    /**
     * S'assure que le point de spawn est bien dans un cul-de-sac (une seule sortie)
     * Si ce n'est pas le cas, trouve un cul-de-sac approprié pour le spawn
     */
    private ensureSpawnIsDeadEnd(): void {
        const directions = [
            { dx: 0, dy: -1 }, // North
            { dx: 1, dy: 0 }, // East
            { dx: 0, dy: 1 }, // South
        ...
        // Compte les sorties du spawn actuel
        let exitCount = 0
        for (const d of directions) {
            const nx = this.spawnPoint.x + d.dx
            const ny = this.spawnPoint.y + d.dy
        ...
        // Si le spawn est déjà un cul-de-sac, on ne fait rien
        if (exitCount === 1) {
            return
        }

        // Sinon, trouve tous les culs-de-sac
        const deadEnds: SpawnPoint[] = []
                ...
                if (this.grid[y][x] === CellType.CORRIDOR) {
                    ...
                    for (let dirIdx = 0; dirIdx < directions.length; dirIdx++) {
                        ...
                        if (ny >= 0 && ny < this.height && nx >= 0 && nx < this.width) {
                            if (this.grid[ny][nx] === CellType.CORRIDOR) {
                                exits++
                                exitDirection = dirIdx
                            }
                        }
                    }

                    // Si c'est un cul-de-sac, ajoute-le
                    if (exits === 1) {
                        // Détermine la direction du joueur (vers la sortie)
                        const playerDirection = exitDirection
                        deadEnds.push({ x, y, direction: playerDirection })
                    }
                }
            }
        }

        // S'il y a des culs-de-sac, choisit-en un
        if (deadEnds.length > 0) {
            const newSpawnIndex = Math.floor(this.random() * deadEnds.length)
            this.spawnPoint = deadEnds[newSpawnIndex]
        }
    }
        ...
        if (deadEnds.length > 0) {
            ...
            for (let y = this.height - 2; y >= 1; y--) {
                for (let x = this.width - 2; x >= 1; x--) {
                    if (this.grid[y][x] === CellType.CORRIDOR && (x !== this.spawnPoint.x || y !== this.spawnPoint.y)) {
                        this.exitPoint = { x, y }
                        return
                    }
                }
     ...
     */
    private carve(x: number, y: number, initialDirection?: number, isFirstCall?: boolean): void {
     ...
     */
    private canCarveWithoutAdjacency(x: number, y: number, direction: Direction): boolean {
