...
export function useGame() {
	...
	function drawExit(): void {
		...
		ctxMinimap.lineTo(
			x + cellSize * 0.25 + poleWidth + flagWidth,
			y + cellSize * 0.1 + flagHeight / 2,
		);
		ctxMinimap.lineTo(
			x + cellSize * 0.25 + poleWidth,
			y + cellSize * 0.1 + flagHeight,
		);
	...
	function drawGame(
		...
		const actualMoveProgress =
			moveDirection === "backward" ? -moveProgress : moveProgress;
	...
	function generateNewDungeon(forcedSpawn?: SpawnPoint): void {
		...
		const seedNum = seedValue ? parseInt(seedValue, 10) : Date.now();

		currentDungeon = dungeonGenerator.generate(seedNum, forcedSpawn);

		if (renderer3D) {
			renderer3D.refreshDungeonColors();
		}

		seed.value = dungeonGenerator.getSeed().toString();
